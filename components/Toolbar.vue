    ...
    <v-app-bar app color="white" class="app-bar-shadow">
      <div class="toolbar-wrapper">
        ...
        <v-btn
          text
          @click="goToCampus"
          class="hidden-sm-and-down text-none font-weight-regular green-btn"
        >
        ...
        <v-menu v-model="drawer" bottom offset-y min-width="150">
          ...
          <v-list nav>
            <v-list-item
              v-for="item in items"
              :key="item.title"
              :to="item.url.startsWith('http') ? null : localePath(item.url)"
              link
...
export default {
  ...
  computed: {
    items() {
      return [
        ...
        {
          title: this.$t("toolbar.privacy"),
          url: "https://es.campus.ai/privacy"
        }
      ];
    }
  },
  methods: {
    ...
    goToCampus() {
      window.open("https://www.campus.ai/es/", "_blank");
    }
  }
};
</script>
