<template>
  <v-container>
    <div v-if="result">
      ...
      <h1>{{ $t("compare.comparision_results") }}</h1>
...
export default {
  ...
  async asyncData({ params, store, $axios }) {
    try {
      const result = await $axios.$get(
        "https://bigfive4ai.campusai.pl/api/" + "compare/" + params.id
      );
      // const result = await $axios.$get(
      //   "http://localhost:4000/api/" + "compare/" + params.id
      // );
      return { result };
    } catch (error) {
  ...
  head: () => ({
    title:
      "Team Comparison of personalities from the Big Five personality traits test",
    meta: [
      {
        name: "keywords",
        content:
      ...
      // Open Graph
      {
        name: "og:title",
        content:
          "See our comparison of personalities from the BigFive personality test"
      },
      {
        name: "og:description",
        content:
          "See our personalities and compare yourself in the following five domains: Neuroticism, Openness To Experience, Conscientiousness, Extraversion, Agreeableness"
      },
      ...
      },
      {
        ...
        content:
          "See our personality comparison in the following five domains: Neuroticism, Openness To Experience, Conscientiousness, Extraversion, Agreeableness"
      },
